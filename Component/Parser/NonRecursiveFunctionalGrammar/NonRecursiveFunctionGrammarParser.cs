    ...
    internal sealed class NonRecursiveFunctionGrammarParser
    {
        ...
        private void AddRuleToGramTree(TreeNode node, IEnumerable<string> ruleTerms, Rule rule)
        {
            ...
            if (rt == null)
                return;
            TreeNode rtNode;
            if (node.SubNodes.TryGetValue(rt, out var subNode))
                rtNode = subNode;
            else
                node.SubNodes.Add(rt, rtNode = new TreeNode(rt));
        ...
        private List<Rule> ReadRule(string rule)
        {
            ...
            foreach (var x in t)
            {
                if (_lexs.TryGetValue(x, out var lex))
                {
                    ...
                    else
                    {
                        ...
                        {
                            var nlex = colex;
                            if (_lexs.TryGetValue(colex, out var trlex))
                                nlex = trlex;
                            AddSeq(nlex);
                        }
                    }
                    ...
                    else
                    {
                        ...
                        foreach (var seq in seqs)
                        {
                            foreach (var or in ors)
                            {
                                ...
                                {
                                    if (_lexs.TryGetValue(x, out var trlex))
                                        sor = trlex;
                                    nseq.Add(sor);
                                }
                                nseqs.Add(nseq);
            ...
            {
                for (var j = 0; j < seqs[i].Count; j++)
                {
                    if (_lexs.TryGetValue(seqs[i][j], out var lx))
                        seqs[i][j] = lx;
                }
            }
            ...
            )
        {
            ...
            var matching = false;
            var partialMatching = false;

            while (i < chars.Length)
            {
                var lastChar = i == chars.Length - 1;
                var c = chars[i];
                var sc = "" + c;
                matching = false;
                var beginIndex = i;
