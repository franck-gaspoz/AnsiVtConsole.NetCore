namespace AnsiVtConsole.NetCore.Component.Console
{
    sealed class InputMap
    {
        ...
        public int Match(string input, ConsoleKeyInfo key)
        {
            if (Text != null)
            {
                if (Text.Equals(input, CaseSensitiveMatch ? StringComparison.CurrentCulture : StringComparison.CurrentCultureIgnoreCase))
                    return ExactMatch;
                if (Text.StartsWith(input, CaseSensitiveMatch ? StringComparison.CurrentCulture : StringComparison.CurrentCultureIgnoreCase))
                    return PartialMatch;
                return NoMatch;
            }
            else
            {
                return (MatchInput == null) ? -1 : MatchInput(input, key);
            }
        }
    }
}
