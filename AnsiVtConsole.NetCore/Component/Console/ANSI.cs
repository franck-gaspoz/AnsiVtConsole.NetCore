    ...
    public static class ANSI
    {
        ...
        public static string? SGRF8(object o)
        {
            ...
            try
            {
                ...
                if (n < 0 || n > 255)
                    throw new Exception("value out of bounds (8its max)");
        ...
        public static string? SGRF24(object o)
        {
            ...
            try
            {
                static int convert(string s)
                {
                    var n = Convert.ToInt16(s);
                    if (n < 0 || n > 255)
                        throw new Exception("value out of bounds (8bits max)");
                    return n;
                }
                var t = s.Split(":");
                if (t.Length != 3)
                    throw new Exception("wrong numbers of parameters.. attempted exactly two comma");
                var r = convert(t[0]);
                var g = convert(t[1]);
                var b = convert(t[2]);
        ...
        public static string? SGRB8(object o)
        {
            ...
            try
            {
                var n = Convert.ToInt16(s);
                if (n < 0 || n > 255)
                    throw new Exception("value out of bounds for 8bits color palette");
        ...
        public static string? SGRB24(object o)
        {
            ...
            try
            {
                ...
                var t = s.Split(":");
                if (t.Length != 3)
                    throw new Exception("wrong numbers of parameters.. attempted exactly two ,");
                var r = convert(t[0]);
                var g = convert(t[1]);
                var b = convert(t[2]);
        ...
        public static string Set4BitsColors(int foregroundNum, int backgroundNum)
        {
            var r = "";
            if (backgroundNum > -1)
                r += $"{CSI}{(((backgroundNum & 0b1000) != 0) ? "4" : "10")}{backgroundNum & 0b111}m";
            if (foregroundNum > -1)
                r += $"{CSI}{(((foregroundNum & 0b1000) != 0) ? "3" : "9")}{foregroundNum & 0b111}m";
            return r;
        }
